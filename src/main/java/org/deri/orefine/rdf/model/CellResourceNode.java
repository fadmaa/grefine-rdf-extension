...
public class CellResourceNode extends ResourceNode implements CellNode{
	...
	public Resource[] createResource(Model model, URI baseUri, Project project, Row row, int rowIndex,
			Resource[] blanks) {
		try{
        	Object result = Utils.evaluateExpression(project, uriExpression, columnName, row, rowIndex);
            if (result.getClass() == EvalError.class){
            	return null;
            }
            if (result.getClass().isArray()) {
            	int lngth = Array.getLength(result);
            	Resource[] rs = new Resource[lngth];
            	for(int i=0;i<lngth;i++){
            		String uri = Utils.resolveUri(baseUri,  Array.get(result, i).toString());
            		rs[i] = model.createResource(uri);
            	}
            	return rs;
            }
            if (result.toString().length() > 0) {
            	String uri = Utils.resolveUri(baseUri, result.toString());
                return new Resource[] { model.createResource(uri) };
            } else {
                return null;
            }
        }catch(Exception e){
